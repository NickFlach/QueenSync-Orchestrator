...
export function Shell({ children }: ShellProps) {
  ...
  const { data: health, isError } = useHealthCheck({ query: { refetchInterval: 10000, queryKey: undefined as never } });
              ...
              return (
                ...
                >
                  <Icon
                    className={cn(
                      "w-4 h-4",
                      isActive ? "text-primary drop-shadow-[0_0_5px_rgba(0,255,255,0.8)]" : "text-muted-foreground group-hover:text-foreground"
                    )}
                  />
                  {item.label}
                </Link>
          ...
          <div className="flex items-center gap-2 text-xs">
            <div
              className={cn(
                ...
                health?.status === "ok" ? "bg-primary text-primary" : "bg-destructive text-destructive"
              )}
            />
            <span className="text-muted-foreground uppercase tracking-wider">
              {health?.status === "ok" ? "System Nominal" : isError ? "Connection Lost" : "Checking..."}
            </span>
          </div>
        </div>
      </aside>
